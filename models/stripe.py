...
class FilterStripe(nn.Conv2d):
    ...
    def prune_in(self, in_mask=None):
        if in_mask is not None:
            assert in_mask.size(0) == self.weight.size(1), "The size of the mask does not match the size of the weight tensor."
            self.weight = Parameter(self.weight[:, in_mask, :, :])
            self.in_channels = in_mask.sum().item()

    def prune_out(self, threshold):
        out_mask = self.FilterSkeleton.abs().sum(dim=(1, 2)) > threshold
        assert out_mask.size(0) == self.weight.size(0), "The size of the mask does not match the size of the weight tensor."
...
class Linear(nn.Linear):
    ...
    def prune_in(self, in_mask=None):
        assert in_mask.size(0) == self.weight.size(1), "The size of the mask does not match the size of the weight tensor."
        self.in_features = in_mask.sum().item()
        self.weight = Parameter(self.weight[:, in_mask])
    ...
    def prune_out(self, mask=None):
        assert mask.size(0) == self.weight.size(0), "The size of the mask does not match the size of the weight tensor."
